...
from impls.m2v_sherpa.adapter import M2vSherpaImpl

FAKE = Path(__file__).parent / "fake_eval_batch.py"


def _impl(*extra: str) -> M2vSherpaImpl:
    return M2vSherpaImpl(cmd=[sys.executable, str(FAKE), *extra])
...
def test_setup_without_config_shows_template(tmp_path):
    impl = M2vSherpaImpl(config_path=tmp_path / "local.toml")
...
def test_setup_flags_nonexistent_paths(tmp_path):
    ...
    cfg.write_text(
        ...
        'tts_venv = "z"\ntts_server_script = "w"\ntts_ref_wav = "v"\n',
        encoding="utf-8")
    impl = M2vSherpaImpl(config_path=cfg)
...
def test_setup_builds_command_from_config(tmp_path):
    ...
    sherpa_model_dir = tmp_path / "sherpa_model_dir"
    ...
    cfg.write_text(
        f'exe = "{exe.as_posix()}"\n'
        f'sherpa_model_dir = "{sherpa_model_dir.as_posix()}"\n'
    ...
    expected_cmd = [
        exe.as_posix(),
        "--sherpa-model-dir", sherpa_model_dir.as_posix(),
