"""Batch adapter: shells out to my-2nd-voice's eval-batch binary (NVIDIA
Nemotron streaming ASR + Marian MT + StyleTTS2).
...
_REQUIRED_KEYS = ("exe", "nemotron_venv", "nemotron_server_script", "mt_dir",
                  "tts_venv", "tts_server_script", "tts_ref_wav")

_TEMPLATE = """\
# impls/m2v_nemotron/local.toml — machine-local paths (git-ignored)
exe = "C:/project/training_ai/my-2nd-voice/target/release/eval-batch.exe"
nemotron_venv = "C:/project/training_ai/my-2nd-voice/.venv-nemo"
nemotron_server_script = "C:/project/training_ai/my-2nd-voice/scripts/nemotron_asr_server.py"
...
# extra_args = ["--nemotron-lang", "vi-VN", "--nemotron-model", "nvidia/nemotron-3.5-asr-streaming-0.6b"]
"""


class M2vNemotronImpl:
    """VoiceImpl adapter around one eval-batch invocation per batch."""

    def __init__(self, name: str = "m2v_nemotron",
    ...
    def setup(self) -> None:
        ...
        self._cmd = [
            str(cfg["exe"]),
            "--nemotron-venv", str(cfg["nemotron_venv"]),
            "--nemotron-server-script", str(cfg["nemotron_server_script"]),
