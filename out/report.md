...
|------|------|---|------|--------------|-------------|---------------|----------------|---------------|----------------|--------|
| 1 | m2v_sherpa | 15 | 0.186 | 0.447 | 0.653 | 0.444 | 297/357 | 735/1084 | 7286/13237 | 1 |
| 2 | m2v_default | 15 | 0.271 | 0.303 | 0.533 | 0.303 | 59586/69619 | 495/1008 | 2930/5064 | 0 |
...
|---|---|---|
| `WER` | ASR transcript vs. reference transcript word error rate (capped at 1.0) | lower is better |
...
- **WER**: `m2v_sherpa` leads at 0.186 vs `m2v_default` at 0.271 (0.085 gap).
- **mt_adequacy**: `m2v_sherpa` leads at 0.447 vs `m2v_default` at 0.303 (0.143 gap).
- **mt_fluency**: `m2v_sherpa` leads at 0.653 vs `m2v_default` at 0.533 (0.120 gap).
- **e2e_adequacy**: `m2v_sherpa` leads at 0.444 vs `m2v_default` at 0.303 (0.141 gap).
- **asr_ms P50**: `m2v_sherpa` is 200.6x faster than `m2v_default` (297 vs 59586 ms).
- **mt_ms P50**: `m2v_default` is 1.5x faster than `m2v_sherpa` (495 vs 735 ms).
- **tts_ms P50**: `m2v_default` is 2.5x faster than `m2v_sherpa` (2930 vs 7286 ms).
- **errors**: `m2v_sherpa`: 1/15 (6.7%).
- **errors**: `m2v_default`: 0/15 (0.0%).
...
### fleurs-001891 — m2v_default (e2e 0.05, WER 1.00)
...
### fleurs-001805 — m2v_default (e2e 0.10, WER 0.60)
...
### fleurs-001714 — m2v_default (e2e 0.10, WER 0.34)
...
### fleurs-001997 — m2v_default (e2e 0.10, WER 0.32)
- ref (vi): tuy nhiên dường như tỷ lệ xdr-tb trong tổng số người mắc bệnh lao vẫn còn ở mức thấp chỉ có 6.000 trong tổng số 330.000 người bị nhiễm bệnh tại một thời điểm cụ thể bất kỳ tại nam phi
...
### fleurs-001835 — m2v_default (e2e 0.10, WER 0.29)
...
### fleurs-001757 — m2v_default (e2e 0.15, WER 0.18)
...
### fleurs-001859 — m2v_default (e2e 0.20, WER 0.22)
